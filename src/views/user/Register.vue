      ...
      async handleSubmit() {
        ...
        if (data) {
          try {
            ...
            data.surname = data.userName
            let res = await register(data)
            if (res.success && res.result.canLogin) {
              this.$router.push({name: 'registerResult', params: {...data}})
            }
          } catch (e) {
            console.log(e)
          }
        ...
        this.form.validateFields(['phoneNumber'], {force: true},
          (err, values) => {
            if (!err) {
              ...
              getSmsCaptcha({phoneNumber: values.phoneNumber, type: 1}).then(res => {
                setTimeout(hide, 1);
